  ...
  return (
    <div className="h-screen w-full flex overflow-hidden font-sans text-slate-800 bg-[#f8fafc]">
      ...
      <aside className="w-64 bg-emerald-900 text-white flex flex-col border-r border-slate-200 shrink-0">
        ...
        <div className="p-4 mt-auto border-t border-emerald-800/50 space-y-3">
          <div className="bg-emerald-800/50 p-4 rounded-xl flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-emerald-500 flex items-center justify-center font-bold text-sm">{user.username.charAt(0).toUpperCase()}</div>
            <div className="flex-1 overflow-hidden">
              <p className="text-xs font-bold text-white truncate">{user.username}</p>
              <p className="text-[10px] text-emerald-300 truncate">{user.role}</p>
            </div>
            <button onClick={handleLogout} className="text-emerald-300 hover:text-white transition" title="Logout">
